...
StepDFS::StepDFS(Graph &g_, vertex_t start)
    : StepTraversal(g_, start), color(num_vertices(g_), 0),
...
StepBFS::StepBFS(Graph &g_, vertex_t start)
    : StepTraversal(g_, start), color(num_vertices(g_), 0),
      out_it(num_vertices(g_)), out_end(num_vertices(g_)) {
  auto idx = boost::get(boost::vertex_index, g);
  q.push(start);
...
std::optional<vertex_t> StepBFS::next() {
  auto idx = boost::get(boost::vertex_index, g);
  while (!q.empty()) {
    vertex_t v = q.front();

    // "Enter"/activate vertex (matches StepDFS' 1 -> 3 transition)
    if (color[idx[v]] == 1) {
      color[idx[v]] = 3;
      return v;
    }

    // Explore neighbors incrementally; return each newly discovered vertex
    if (color[idx[v]] == 3) {
      for (; out_it[idx[v]] != out_end[idx[v]]; ++out_it[idx[v]]) {
        edge_t e = *out_it[idx[v]];
        vertex_t u = target(e, g);
